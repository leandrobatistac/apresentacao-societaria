...
function totCell(v, isPct, isRes, opts = {}) {
  return (
    <td style={{
...
// ── ACUM ROW — componente real, hook seguro ───────────────
function AcumRow({ o, gRec, gDesp, gRes, gMargin, pRec, pDesp, pRes, pMargin }) {
...
// ── PREV ROW — componente real, hook seguro ───────────────
function PrevRow({ o, gRec, gDesp, gRes, gMargin, pRec, pDesp, pRes, pMargin }) {
  const [hov, setHov] = useState(false)
  const bg = hov ? 'var(--surface2)' : 'var(--surface)'
  const td = makeTd(bg)
  ...
  return (
    <div>
      <table style={{ width:'100%', borderCollapse:'separate', borderSpacing:0, tableLayout:'auto' }}>
        <TableHead />
        <tbody>
          {rowsData.map(row => <AcumRow key={row.o.num} {...row} />)}
          <tr>
            <td colSpan={5} style={{
  ...
  return (
    <div>
      <table style={{ width:'100%', borderCollapse:'separate', borderSpacing:0, tableLayout:'auto' }}>
        <TableHead />
        <tbody>
          {rowsData.map(row => <PrevRow key={row.o.num} {...row} />)}
          <tr>
            <td colSpan={5} style={{
              padding:'7px 14px', fontWeight:700, fontSize:11, color:'var(--text)', textAlign:'center',
              borderTop:'1px solid var(--border)', borderBottom:'1px solid var(--border)',
